...
```{r dailydata, results="asis", message=FALSE}
...
ggsave("figure/stephist.png")

meanstep <- mean(dailysteps$steps)
medianstep <- median(dailysteps$steps)
```

<b>Mean Daily Steps: </b> `r sprintf("%.2f", meanstep)` <br> 
<b>Median Daily Steps: </b> `r medianstep`
                    
![Figure 1](figure/stephist.png) 

## What is the average daily activity pattern?
The activity data is grouped by interval and means calculated

```{r intervaldata, results="asis", message=FALSE}
...
g <- ggplot(intervalsteps, aes(interval, mean)) +
    ...
    geom_line()
ggsave("figure/intervalavg.png")

maxintmean <- max(intervalsteps$mean)
maxinterval <- filter(intervalsteps, mean == maxintmean)$interval
```

<b>Interval with Maximum Average Steps: </b>`r maxinterval`
(Value: `r maxintmean`)

![Figure 2](figure/intervalavg.png)  
...
```
<b>Intervals Without Steps: </b>`r nacount`

We will impute steps taken for missing values by taking the floor of the mean steps taken for the interval over all days observed.
```{r imputing}
imputesteps <- function (int) {
    as.integer(intervalsteps[intervalsteps$interval==int,]$mean) 
}
...
```{r newstats, message=FALSE}
newdailysteps <- summarise(group_by(actdatanew, date), 
                        steps = sum(newsteps, na.rm = TRUE))
...
g <- ggplot(newdailysteps, aes(steps)) +
    ...
    ggtitle("Histogram of Daily Steps Taken") + 
    geom_histogram(bins=12)
ggsave("figure/newstephist.png")

newmeanstep <- mean(newdailysteps$steps)
newmedianstep <- median(newdailysteps$steps)
```

<b>New Mean Daily Steps: </b> `r sprintf("%.2f", newmeanstep)` <br> 
<b>New Median Daily Steps: </b> `r newmedianstep`

![Figure 3](figure/newstephist.png)
...
g <- ggplot(wdaymean, aes(interval, meansteps)) +
    ...
    geom_line()
ggsave("figure/wday.png")
```
    
![Figure 4](figure/wday.png)  
<BR>
Comparing weekday and weekend activity yields the following:
<UL>
 <LI>Highest activity levels on weekends and weekdays are between 8AM and 9AM 
 <LI>Activity levels between 8AM and 9AM are higher on weekdays
